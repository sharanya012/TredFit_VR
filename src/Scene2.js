...
const Scene = ({ selectedAudio }) => {
  ...
  /*  const startActivity = () => {
        // Initialize audio listener and sound on user interaction
        if (!audioLoaded) {
            const listener = new AudioListener();
            listenerRef.current = listener;

            const audioLoader = new AudioLoader();
            audioLoader.load('/mp3file.mp3', (buffer) => {
                const sound = new Audio(listener);
                sound.setBuffer(buffer);
                sound.setLoop(true);
                sound.setVolume(0.5);
...
const Scene2 = ({ selectedAudio }) => {
    const [isActive, setIsActive] = useState(false);
    const listenerRef = useRef();
    const soundRef = useRef();
    const [audioLoaded, setAudioLoaded] = useState(false);

    // Load audio based on the selected audio prop
    useEffect(() => {
        if (!audioLoaded && selectedAudio) {
            const listener = new AudioListener();
            listenerRef.current = listener;
    ...
    return (
        <div>
            <Canvas style={{ height: '100vh', backgroundColor: '#f0f0f0' }} shadows>
                ...
                {listenerRef.current && <primitive object={listenerRef.current} />}
                <Lights />
                <TreadmillModel positionZ={0} speed={0.005} isActive={isActive} updateDistance={() => {}} />
                <OrbitControls maxPolarAngle={Math.PI / 2} enableDamping={true} dampingFactor={0.25} />
            </Canvas>
            <button onClick={startActivity}>Start Workout</button>
            <button onClick={stopActivity}>Stop Workout</button>
        </div>
    );
};
